package a_variables.tiposdatos;

/**
 * Clase de prueba para crear un programa básico en java
 */
public class B_comentarios {

    //COMENTARIOS JAVA DOC
    /**
     * Punto de entrada a la aplicación java
     * Permite ejecutar la aplicación
     * @param args - argumentos de entrada para la aplicación
     */
    public static void main(String[] args){

        // esto es un comentario
        System.out.println("Adios mundo cruel!"); // esto es otro comentario
        /*
        Esto es un comentario multilínea:
        1 - Crear funcion suma
        2 - Crear variables tipo numérico
        3 - Utilizar la función suma para calcular la suma de los dos números
         */
    }

}
